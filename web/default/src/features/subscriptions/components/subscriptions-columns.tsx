...
export function useSubscriptionsColumns(): ColumnDef<PlanRecord>[] {
  ...
  return useMemo(
    (): ColumnDef<PlanRecord>[] => [
      ...
      {
        id: 'owner_group',
        meta: { label: t('Owner Group'), mobileHidden: true },
        header: ({ column }) => (
          <DataTableColumnHeader column={column} title={t('Owner Group')} />
        ),
        cell: ({ row }) => {
          const group = row.original.plan.owner_group
          return (
            <span className='text-muted-foreground'>
              {group || t('Available to all groups')}
            </span>
          )
        },
        size: 120,
